...
def train_long(model, X_train, y_train, epochs=50, lr=0.005, name="Model"):
    ...
    for e in range(epochs):
        ...
        if hasattr(model, 'set_temperature'):
             model.set_temperature(max(0.1, 5.0 * (0.95 ** e)))
             
    return time.time() - start

def run_universal_tuned():
    print(f"=== Universal SOTA Benchmark (Meta-CDKAN) ===")
    ...
    for asset, data in datasets.items():
        ...
        # 2. CD-KAN (Meta-Optimal)
        cdkan = CDKANForecaster.create_best(in_features=in_f, hidden_dim=64, dataset_name=asset).to(DEVICE)
        train_long(cdkan, X_tr, y_tr, epochs=50, lr=0.01, name="CD-KAN")
        with torch.no_grad():
    ...
    print(df)
    df.to_csv("benchmark_universal_tuned.csv", index=False)
